...
public:
	int GetFilesNum();
	bool WriteFile(CString filePath, void* pBuf,int length, int iLevel = -1);
...
*/
	...
	bool AddNewFile(LPCTSTR lpszFilePath, LPCTSTR lpszSubFilePath, int iLevel = -1, bool bFullPath = true, ZIPCALLBACKFUN pCallback = NULL, void* pUserData = NULL, unsigned long nBufSize = 65535);
...
//		the name of the file to be found
	int FindFile(CString szFileName);
...
protected:
	...
	DWORD DeleteInternalList(CWordArray& uIndexs);
...
protected:
	...
	bool getXML(streamVector* strmVec, IStream** pStream);
	bool FixSplitFiles(storage oDuplicates);
	void FindDuplicates();
};
